...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] RPCPackagerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is RPCPackagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (RPCPackagerNetworkObject.Length > 0 && RPCPackagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(RPCPackagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<RPCPackagerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateRPCPackager instead, its shorter and easier to type out ;)")]
		public RPCPackagerBehavior InstantiateRPCPackagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RPCPackagerNetworkObject[index]);
			var netBehavior = go.GetComponent<RPCPackagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<RPCPackagerBehavior>().networkObject = (RPCPackagerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of RPCPackager
		/// </summary>
		/// <returns>
		/// A local instance of RPCPackagerBehavior
		/// </returns>
		/// <param name="index">The index of the RPCPackager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public RPCPackagerBehavior InstantiateRPCPackager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RPCPackagerNetworkObject[index]);
			var netBehavior = go.GetComponent<RPCPackagerBehavior>();
